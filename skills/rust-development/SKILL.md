...
- 主线逻辑写成最直观的流水线，边缘情形剥离为独立分支，不与主线交织。
...
- 改动范围以需求为界；本次触碰的代码需完全符合本文，未触碰的不得顺手修改。
- 交付前跑 `clippy`，清空本次改动文件的警告。
- 调试输出、测试、基准等开发期设施对主线最小侵入，不改动签名与可见性。
...
- 命名以 Rust 通行约定为底线，其上公共抽象对齐 `std`、业务逻辑对齐项目既有；既有名字违反底线的不作参照。
- 名字自身完成消歧义，且准确到不会被误用或误改；需靠注释补说才能正确使用的名字，改名而不是补注释。
- 承担同一角色的项必须同名。
- 处于同一并列地位的项——枚举变体、方法组、关联常量、字段组——词性与构词形式保持一致。
- 同一语义的多种长度形态用 `std` 后缀区分：`_chunk`、`_array`、`_slice`、`_iter`。
- 仅有一个调用点、实际逻辑不过一两行的辅助项不独立命名，内联到使用处。
...
- 值在真正使用它的那一处从权威来源取，不提前取出沿调用链透传。
- 分支要产出值时用 `match` 或 `if else` 表达式，不写连续 `return`。
- 只有被判定的对象天然是二值时才用 `if else`，其余一律 `match`；不因当前只有两种取值就写 `if else`。
...
- 能用适配器或 `std` 切分器（`as_chunks`、`chunks_exact` 等）组合表达的不手写循环。
- 能用 `for` 或 `while` 表达的循环不用 `loop`；无条件循环除外。
- 不用递归代替循环。
...
- 由上游保证的前提用断言表达，不写成 `if` 兜底返回的形式。
- 前提能由类型排除的就不写断言，改用 newtype、`NonZero`、`NonNull`、`where` 上的长度表达式等类型手段。
- 断言写在它所证明的前提必须成立之处，推论处一般不再重述：入口前提在函数首行，语句或代码块的前提紧贴其前，循环不变量在循环内。
- 断言要写下游所需性质的充分条件而非必要条件。
